...
describe('Component: DatabaseViewer', () => {
    ...
    test('DatabaseViewer calls filterTable', () => {

        const filterTableSpy = jest.spyOn(DatabaseViewer, "filterTable");
        component = shallow(<DatabaseViewer {...props}/>);
        expect(filterTableSpy).toHaveBeenCalledTimes(1);

        component.setProps({searchProps: "_pg"});

        expect(filterTableSpy).toHaveBeenCalledTimes(2);

        component.setProps({searchProps: "_pg "});

        expect(filterTableSpy).toHaveBeenCalledTimes(3);
    })
});